/* $NetBSD: hpcIo.c,v 1.1 2004/01/03 01:09:19 takemura Exp $	*/
...
hpcEnqueueEvents ()
{
    hpcEvent	*ptrEvents,    	/* Current pointer event */
    ...
    hpcKbdPrivPtr       kbdPriv;
    hpcPtrPrivPtr       ptrPriv;

    pPointer = (DeviceIntPtr)LookupPointerDevice();
    pKeyboard = (DeviceIntPtr)LookupKeyboardDevice();
    ptrPriv = (hpcPtrPrivPtr) pPointer->public.devicePrivate;
    kbdPriv = (hpcKbdPrivPtr) pKeyboard->public.devicePrivate;
	...
	if ((numPtrEvents == 0) && PtrAgain) {
	    ptrEvents = hpcMouseGetEvents (ptrPriv, &nPE, &PtrAgain);
	    numPtrEvents = nPE;
	}
	if ((numKbdEvents == 0) && KbdAgain) {
	    kbdEvents = hpcKbdGetEvents (kbdPriv, &nKE, &KbdAgain);
	    numKbdEvents = nKE;
	}
	...
	if (numPtrEvents && numKbdEvents) {
		...
		hpcKbdEnqueueEvent(pKeyboard, kbdEvents);
		numKbdEvents--;
		kbdEvents++;
	    } else {
		hpcMouseEnqueueEvent(pPointer, ptrEvents);
		numPtrEvents--;
		ptrEvents++;
	    }
