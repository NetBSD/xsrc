/*	$NetBSD: lk201.c,v 1.1 2004/01/10 05:23:23 rtr Exp $	*/
...
/*
 * XXX - fixed for Right Shift on LK501 [and possibly LK401].  The Right
 *       Shift keycode is 171 [0xab] was in division 8 with the up/down
 *       arrows [169/170 0xa9/0xaa].  This caused the Right Shift to
 *       be processed as a "normal" key rather than an up/down modifier.
 *       This fix adjusts the division table to move the Right Shift
 *       key into division 6 (with shift and control [174/175 0xae/0xaf]).
 *       This is done by changing the start of division 6 from 0xad to
 *       0xab, and the end of division 8 from 0xac to 0xaa.  Also, the
 *       initial state for up/down keys includes the bit for Right Shift.
 *       NOTE:  this also requires a corresponding change in the NetBSD
 *              dtop.c division/keyhmodes tables.
 */
#define NUMDIVS 14
...
static unsigned int keymodes[8] = {0, 0, 0, 0, 0, 0x0001c800, 0, 0};
	/* down/up keys */
...
#define INDEX(in) ((in - MIN_LK201_KEY) * LK201_GLYPHS_PER_KEY)
    ...
    map[INDEX(KEY_R_SHIFT)] = XK_Shift_R;
