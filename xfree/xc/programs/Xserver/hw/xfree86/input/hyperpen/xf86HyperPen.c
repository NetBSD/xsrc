...
/* Clear any pending input */
    ...
    xf86Msg(X_PROBED, "%s: HyperPen Model: 0x%x (%s)\n", local->name,
    ...
    xf86Msg(X_PROBED, "%s: HyperPen max tablet size %d.%02dinx%d.%02din, %dx%d "
	       "lines of resolution\n",
	       local->name,
	...
	if ((priv->hypXSize * res100) <= priv->hypMaxX &&
	    ...
	    xf86Msg(X_PROBED, "%s: HyperPen active area bigger than tablet, "
		   "assuming maximum\n", local->name);
    ...
    xf86Msg(X_PROBED,
	    "%s: HyperPen using tablet area %d by %d, at res %d lpi\n",
	    local->name, priv->hypXSize, priv->hypYSize, priv->hypRes);
