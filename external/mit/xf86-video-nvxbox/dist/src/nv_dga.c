...
NV_SetMode(
   ...
   if(!pMode) { /* restore the original mode */
      ...
      NVXSwitchMode(index, pScrn->currentMode, 0);
      NVXAdjustFrame(index, pScrn->frameX0, pScrn->frameY0, 0);
      pNv->DGAactive = FALSE;
   } else {
	...
	pNv->DGAactive = TRUE;
      ...
      NVXSwitchMode(index, pMode->mode, 0);
   }
...
NV_SetViewport(
   ...
   NVXAdjustFrame(pScrn->pScreen->myNum, x, y, flags);
