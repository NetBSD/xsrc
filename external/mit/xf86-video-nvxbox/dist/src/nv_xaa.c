...
{    
    if (pNv->currentRop != rop)
    {
        ...
        pNv->riva.Rop->Rop3 = XAAGetCopyROP(rop);
    }
}
...
NVSetRopPattern(NVPtr pNv, int rop)
{
    if (pNv->currentRop != rop + 16)
    {
        ...
        RIVA_FIFO_FREE(pNv->riva, Rop, 1);
        pNv->riva.Rop->Rop3 = XAAGetPatternROP(rop);
    }
}
...
void
NVXResetGraphics(ScrnInfoPtr pScrn)
{
    NVPtr pNv = NVPTR(pScrn);
...
Bool
NVXAccelInit(ScreenPtr pScreen) 
{
