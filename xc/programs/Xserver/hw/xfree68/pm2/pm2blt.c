/* $XFree86: xc/programs/Xserver/hw/xfree68/pm2/pm2blt.c,v 1.1.2.1 1999/06/02 07:50:15 hohndel Exp $ */
/*
...
static RegionPtr pm2fbBitBlt();
static void pm2fbFindOrdering();
static void pm2fbCopyPlane1toN();
static void pm2fbCopyPlaneNto1();
...
RegionPtr
pm2fbCopyArea(pSrcDrawable, pDstDrawable,
    ...
    int dstx, dsty;
{
	...
	switch (max(pSrcDrawable->bitsPerPixel, pDstDrawable->bitsPerPixel))
	{
		...
		return cfb32CopyArea(pSrcDrawable, pDstDrawable, pGC,
			   	     srcx, srcy, width, height, dstx, dsty);
	    default:
		ErrorF("pm2fbCopyArea: unsupported depth %d \n",
    ...
    {
        pRgnExposed = pm2fbBitBlt (pSrcDrawable, pDstDrawable, pGC, 
				    srcx, srcy, width, height, dstx, dsty,
                                    pm2fbDoBitBlt, 0);
        return pRgnExposed;
    }
...
RegionPtr
pm2fbCopyPlane(pSrcDrawable, pDstDrawable,
    ...
    unsigned long	bitPlane;
{
	...
	if (bitPlane == 1)
	{
	    ret =  pm2fbBitBlt (pSrcDrawable, pDstDrawable, 
				 pGC, srcx, srcy, width, height, 
				 dstx, dsty, (void *)pm2fbCopyPlane1toN,
				 bitPlane);
	}
	else
	{
	...
	(void) pm2fbBitBlt (pSrcDrawable, (DrawablePtr) pBitmap,
                             pGC1, srcx, srcy, width, height, 0, 0, 
                             pm2fbCopyPlaneNto1, bitPlane);

        /* no exposures here, copy bits from inside a pixmap */
        (void) pm2fbBitBlt ((DrawablePtr) pBitmap, pDstDrawable, pGC,
                             0, 0, width, height, dstx, dsty, 
			     pm2fbCopyPlane1toN, 1);
...
void
pm2fbGetImage(pDrawable, sx, sy, w, h, format, planeMask, pdstLine)
...
/*#ifdef DEBUG
        ErrorF("pm2fbGetImage: launching DoBitBlt\n");
#endif*/
        pm2fbDoBitBlt(pDrawable, (DrawablePtr)pPixmap, &dummyGC, &rgnDst,
                       &ptSrc, planeMask);
	...
	switch (pDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
		ErrorF("pm2fbGetImage: unsupported depth %d \n",
...
static RegionPtr
pm2fbBitBlt (pSrc, pDst, pGC, srcX, srcY, copyWidth, copyHeight, 
...
static void
pm2fbFindOrdering(pSrcDrawable, pDstDrawable, pGC, numRects, boxes, 
...
void
pm2fbDoBitBlt (pSrc, pDst, pGC, rgnDst, pptSrc, bitPlane)
    DrawablePtr	  pSrc;		/* Pointer to source drawable */
    DrawablePtr	  pDst;		/* Pointer to destination drawable */
    GCPtr	  pGC;		/* Pointer to current GC */
				   ...
				   function vector parameter list */
{
	...
	dy = pptSrc->y - pbox->y1;
        ...
        pm2fbFindOrdering(pSrc, pDst, pGC, numRects, pbox, 
			   pptSrc->x, pptSrc->y, 
...
/*#ifdef DEBUG
	ErrorF("pm2fbDoBitBlt: setting up...");
#endif*/

	(pm2fbSetupForScreenToScreenCopy)(xdir, ydir, pGC->alu, pGC->planemask, 0);
...
/*#ifdef DEBUG
        ErrorF("pm2fbDoBitBlt: Copying from pixmap to window...");
#endif*/
	while(numRects --)
	{
	    {
		(pm2fbImageWriteFunc)(pbox->x1, pbox->y1,
				  pbox->x2 - pbox->x1, 
				  pbox->y2 - pbox->y1,
...
/*#ifdef DEBUG
        ErrorF("pm2fbDoBitBlt: Copying from window to pixmap...");
#endif*/
	    while(numRects --)
	    {
	        (pm2fbImageReadFunc)(pptSrc->x, pptSrc->y,
			              pbox->x2 - pbox->x1, 
				      pbox->y2 - pbox->y1,
			 	      pdst, pixWidth, 
				      pbox->x1, pbox->y1,
	...
	{
/*#ifdef DEBUG
	    ErrorF("pm2fbDoBitBlt: Falling back to cfbDoBitBlt\n");
#endif*/
	    switch (max(pSrc->bitsPerPixel, pDst->bitsPerPixel))
	    {
		    ...
		    break;
		default:
		    ErrorF("pm2fbDoBitBlt: unsupported depth %d \n",
    ...
    {
	ErrorF("pm2fbDoBitBlt: pixmap to pixmap copy not supported\n");
    }
    return;
...
static void
pm2fbCopyPlane1toN(pSrcDrawable, pDstDrawable, pGC, rgnDst, pptSrc, bitPlane)
    DrawablePtr   pSrcDrawable;
    DrawablePtr   pDstDrawable;
    GCPtr         pGC;
    RegionPtr     rgnDst;
    ...
    unsigned long bitPlane;
{
	...
	(pDstDrawable->type == DRAWABLE_WINDOW) && xf86VTSema)
    {
        ...
        {
            pm2fbImageStippleFunc(pbox->x1, pbox->y1,
                                   pbox->x2 - pbox->x1, 
			           pbox->y2 - pbox->y1,
                                   psrc, pixWidth,
                                   pptSrc->x, pptSrc->y, 
	...
	switch (pDstDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
	        ErrorF("pm2fbCopyPlane1toN: unsupported function \n");
		break;
	}
    }
}
...
static void
pm2fbCopyPlaneNto1(pSrcDrawable, pDstDrawable, pGC, rgnDst, pptSrc, bitPlane)
    DrawablePtr   pSrcDrawable;
    DrawablePtr   pDstDrawable;
    GCPtr         pGC;
    RegionPtr     rgnDst;
    ...
    unsigned long bitPlane;
{
	...
	case 32:
	    ...
	    break;
	default:
	    ErrorF("pm2fbCopyPlaneNto1: unsupported function \n");
	    break;
    }
}
