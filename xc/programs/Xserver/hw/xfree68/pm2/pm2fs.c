/* $XFree86: xc/programs/Xserver/hw/xfree68/pm2/pm2fs.c,v 1.1.2.1 1999/06/02 07:50:18 hohndel Exp $ */
/************************************************************
...
void
pm2fbSolidFSpans (pDrawable, pGC, nInit, pptInit, pwidthInit, fSorted)
    ...
    int         fSorted;
{
	...
	switch (pDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
		ErrorF("pm2fbSolidFSpans: unsupported depth %d \n",
...
#ifdef DEBUG
	ErrorF("pm2fbSolidFSpans: here we go\n");
#endif
    ...
    while (n--) 
    {
        ...
        Permedia2SubsequentFillRectSolid(ppt->x, ppt->y, *pwidth, 1);
...
void
pm2fbTiledFSpans (pDrawable, pGC, nInit, pptInit, pwidthInit, fSorted)
    DrawablePtr pDrawable;
    GCPtr       pGC;
    int         nInit;                  /* number of spans to fill */
    DDXPointPtr pptInit;                /* pointer to list of start points */
    ...
    int         fSorted;
{
	...
	switch (pDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
		ErrorF("pm2fbTiledFSpans: unsupported depth %d \n",
			pDrawable->bitsPerPixel);
	}
	return;
    }
...
void
pm2fbStipFSpans (pDrawable, pGC, nInit, pptInit, pwidthInit, fSorted)
    DrawablePtr pDrawable;
    GCPtr       pGC;
    int         nInit;                  /* number of spans to fill */
    DDXPointPtr pptInit;                /* pointer to list of start points */
    ...
    int         fSorted;
{
	...
	switch (pDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
		ErrorF("pm2fbStipFSpans: unsupported depth %d \n",
			pDrawable->bitsPerPixel);
	}
	return;
    }
...
void
pm2fbOStipFSpans (pDrawable, pGC, nInit, pptInit, pwidthInit, fSorted)
    DrawablePtr pDrawable;
    GCPtr       pGC;
    int         nInit;                  /* number of spans to fill */
    DDXPointPtr pptInit;                /* pointer to list of start points */
    ...
    int         fSorted;
{
	...
	switch (pDrawable->bitsPerPixel)
	{
		...
		break;
	    default:
		ErrorF("pm2fbOStipFSpans: unsupported depth %d \n",
			pDrawable->bitsPerPixel);
	}
	return;
    }
    ...
    int         *pwidthInit;            /* pointer to list of n widths */
{
    ...
    nullBox.x1 = 0;
    nullBox.y1 = 0;
    nullBox.x2 = pm2fbVirtX;
    nullBox.y2 = pm2fbVirtY;
    ...
    pm2fbPolyFillRect (pDrawable, pGC, nInit, pRectInit);
