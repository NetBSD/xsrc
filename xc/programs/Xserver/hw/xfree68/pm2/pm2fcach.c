/* $XFree86: xc/programs/Xserver/hw/xfree68/pm2/pm2fcach.c,v 1.1.2.1 1999/06/02 07:50:16 hohndel Exp $ */
/*
...
CacheFontRec	pm2fbFontCache[MAX_FONTS];
static int      NumFonts;


void
pm2fbFontCache8Init(x,y)
    int x, y;
{
    int        i;
    int        free_ram    = glintInfoRec.videoRam * 1024
                             - x * y * (glintInfoRec.bitsPerPixel / 8);
    int        lines       = min(2048-pm2fbMaxY,
                                free_ram / (x * (glintInfoRec.bitsPerPixel / 8)));
    int        cache_sets;
    static int initialized = 0;

#ifndef NO_FONT_CACHE
    if (OFLG_ISSET(OPTION_NO_FONT_CACHE, &glintInfoRec.options))
#endif
    {
	if (!initialized) {
    ...
    if (!initialized) {
       ...
       if (xf86Verbose)
	     ErrorF("%s %s: Font cache: %d fonts\n", XCONFIG_PROBED,
		    glintInfoRec.name, NumFonts );
    }
    for (i = 0; i < NumFonts; i++) {
	pm2fbFontCache[i].font = (FontPtr)0;
	pm2fbFontCache[i].lru = 0xffffffff;
	pm2fbFontCache[i].x = 256 + (96 * (i % 8));
	pm2fbFontCache[i].y = y + 256 * (i >> 3);
    }
}
 ...
 */
static void
pm2fbCacheGlyph(x, y, width, height, psrc, pwidth)
    int                 x;
    int                 y;
    int                 width;
    int                 height;
    ...
    int                 pwidth;
{
    ...
    offset = (((y * pm2fbVirtX) + x) * (glintInfoRec.bitsPerPixel / 8)) >> 3;
    ...
    FontPtr font;
{
	...
	(FONTLASTCOL(font) < 256)) {
        ...
        int pm2fbFC_X, pm2fbFC_Y;
	
	ret = 0;
	for (i = 1; i < NumFonts; i++)
	    if (pm2fbFontCache[i].lru > pm2fbFontCache[ret].lru)
		ret = i;
	...
	pm2fbFontCache[ret].font = font;
	pm2fbFontCache[ret].lru = 0;
	pm2fbFC_X = pm2fbFontCache[ret].x;
	pm2fbFC_Y = pm2fbFontCache[ret].y;

	for (c = 0; c < 256; c++) {
	    chr = (unsigned char)c;
	    GetGlyphs(font, 1, &chr, Linear8Bit, &n, &pci);
	    if (n == 0) {
		pm2fbFontCache[ret].pci[c] = NULL;
	    } else {
		pm2fbFontCache[ret].pci[c] = pci;
		...
		if (gWidth && gHeight) {
		    ...
		    pm2fbCacheGlyph(pm2fbFC_X, (pm2fbFC_Y + c),
...
int
pm2fbDrawText(pDraw, pGC, x, y, count, chars, slot, texttype)
    DrawablePtr pDraw;
    GCPtr pGC;
    int x;
    int y;
...

{
    ...
    else
    {
        ...
        {
            width += pm2fbFCP->pci[(int)chars[i]] ?
                     pm2fbFCP->pci[(int)chars[i]]->metrics.characterWidth : 0;
        }
    }
    ret_val = x + width;
    ...
    for (i = 0; i < NumFonts; i++)
    {
        if (i != slot && pm2fbFontCache[i].lru != 0xffffffff)
            pm2fbFontCache[i].lru++;
    }
    ...
    if (texttype & IMAGE_TEXT_TYPE)
    {
	    ...
	    Permedia2SetupForFillRectSolid(pGC->bgPixel, GXcopy, ~0L);
	...
	if ((fontAscent > rectY2) || (fontDescent < rectY1) ||
	    ...
	    offset = ((((pm2fbFCP->y + (int)chars[i]) * pm2fbVirtX) +
			pm2fbFCP->x) * (glintInfoRec.bitsPerPixel / 8)) >> 3;
