/* $XFree86: xc/programs/Xserver/hw/xfree68/pm2/pm2pntwn.c,v 1.1.2.1 1999/06/02 07:50:21 hohndel Exp $ */
/***********************************************************
...
static void pm2fbFillBoxSolid();
static void pm2fbFillBoxTile();

void
pm2fbPaintWindow(pWin, pRegion, what)
    WindowPtr	pWin;
    RegionPtr	pRegion;
    int		what;
{
	...
	case BackgroundPixmap:
	    pm2fbFillBoxTile ((DrawablePtr)pWin, pRegion,
		               pWin->background.pixmap);
	    return;
	case BackgroundPixel:
	    pm2fbFillBoxSolid ((DrawablePtr)pWin, pRegion,
...
static void
pm2fbFillBoxSolid (pDrawable, pRegion, pixel)
    ...
    unsigned long   pixel;
{
    ...
    Permedia2SetupForFillRectSolid(pixel, GXcopy, ~0L);
    ...
    for (; nBox; nBox--, pBox++) {
        ...
        if ((w >= 0) && (h >= 0)) {
            ...
            Permedia2SubsequentFillRectSolid(pBox->x1, pBox->y1, w, h);
        }
    }
...
static void
pm2fbFillBoxTile (pDrawable, pRegion, pPixmap)
    DrawablePtr     pDrawable;
    RegionPtr       pRegion;
    PixmapPtr       pPixmap;
{
