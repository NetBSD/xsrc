% $XConsortium: dbe.tex /main/5 1996/01/19 15:03:46 converse $
...
\newcommand{\typename}[1]{\mbox{\rm #1}}
\newcommand{\argname}[1]{\mbox{\it #1}}
\newcommand{\argdecl}[2]{\argname{#1} & : \typename{#2}\\}

\newenvironment{arequest}[1]{\requestname{#1} \\ \begin{tabular}{ll}}{\end{tabular}}
...
\newenvironment{erequest}[3]{\pagebreak[3] \begin{keeptogether} \requestname{#1} \begin{tabbing} \etabstops \eargdecl{1}{CARD8}{major-opcode} \eargdecl{1}{#2}{minor-opcode} \eargdecl{2}{#3}{request length}}{\end{tabbing} \end{keeptogether}}
...
contain a \typename{DRAWABLE} (e.g., \literal{GraphicsExpose}) are
generated in
...
window background in both buffers, and \literal{Expose} events are
generated for D.
...
\areply
\argdecl{server-major-version}{CARD8}
\argdecl{server-minor-version}{CARD8}
\end{arequest}

The \argname{client-major-version} and \argname{client-minor-version}
numbers indicate what version of the protocol the client wants the
server to implement.  The \argname{server-major-version} and the
\argname{server-minor-version} numbers returned indicate the protocol
...
than one version simultaneously.  The \argname{server-major-version}
and \argname{server-minor-version} allow the creation of future
...
in this document will return a \argname{server-major-version} of one
(1), and a \argname{server-minor-version} of zero (0).

The Double Buffering client must issue a \requestname{DBEGetVersion}
...
\areply
\argdecl{visinfo}{LISTofSCREENVISINFO}
\end{arequest}
...
\typename{VISINFO}: \> [ \= \typename{VISUALID} \argname{visual};\\
			\>\> \typename{CARD8} \argname{depth};\\
			\>\> \typename{CARD8} \argname{perflevel} ]\\
\end{tabbing}

All of the values passed in \argname{screen-specifiers} must be valid
\typename{DRAWABLE}s (or a \literal{Drawable} error results).  For each drawable
in \argname{screen-specifiers}, the reply will contain a list of
VISINFO structures for visuals that support double-buffering on the
screen on which the drawable resides.  The \argname{visual} member
specifies the VISUALID\@.  The \argname{depth} member specifies the
depth in bits for the \argname{visual}.  The \argname{perflevel} is a
performance hint.  The only operation defined on a \argname{perflevel}
is comparison to a \argname{perflevel} of another visual on the same
screen.  The visual having the higher \argname{perflevel} is likely to
have better double-buffer graphics performance than the visual having
the lower \argname{perflevel}.  Nothing can be deduced from any of the
following: the magnitude of the difference of two
\argname{perflevel}s, a \argname{perflevel} value in isolation, or
comparing \argname{perflevel}s from different servers.

If the list of \argname{screen-specifiers} is empty, information for
...
If the \argname{window} is not already a double-buffered window, the
window becomes double-buffered, and the \argname{back-buffer-name} is
associated with the window.  The \argname{swap-action-hint} tells the
server which swap action is most likely to be used with the window in
subsequent \requestname{DBESwapBuffers} requests.  The
\argname{swap-action-hint} must have one of the values specified for
type \typename{SWAPACTION} (or a Value error results).  See the
...
window changes, except that an additional \argname{back-buffer-name}
...
The \argname{back-buffer-name} must be in the range assigned to the
...
remains unchanged, and the \argname{back-buffer-name} is freed.
...
The \argname{back-buffer-name} passed in the request is freed and no
longer associated with the window.  If this is the last
\argname{back-buffer-name} associated with the window, then the back
...
The \argname{back-buffer-name} must be a valid \typename{BACKBUFFER}
...
\typename{SWAPINFO}: [\typename{WINDOW}
\argname{window}; \typename{SWAPACTION} \argname{swap-action}]
...
Each \argname{swap-action} in the list must have one of the
...
The \argname{swap-action} determines what will happen to the new back
buffer of the \argname{window} it is paired with in the list in
...
\argdecl{back-buffer-name}{BACKBUFFER}
\areply
\argdecl{attributes}{BUFFER\_ATTRIBUTES}
\end{arequest}

The following type definitions apply:

\typename{BUFFER\_ATTRIBUTES}: [\typename{WINDOW} \argname{window}; ]

If \argname{back-buffer-name} is a valid \typename{BACKBUFFER}, the
\argname{window} field of the \argname{attributes} in the reply will
be the window which has the back buffer that
\argname{back-buffer-name} refers to.  If \argname{back-buffer-name}
is not a valid \typename{BACKBUFFER}, the \argname{window} field of
the \argname{attributes} in the reply will be \literal{None}.
...
This function sets \argname{major\_version\_return} and
\argname{minor\_version\_return} to the major and minor DBE protocol
...
non-zero.  If \argname{dpy} does not support the DBE extension, or if
...
buffering.  The argument \argname{num\_screens} specifies how many
elements there are in the \argname{screen\_specifiers} list.  Each
drawable in \argname{screen\_specifiers} designates a screen for which
the supported visuals are being requested.  If \argname{num\_screens}
is zero, information for all screens is requested.  In this case, upon
return from this function, \argname{num\_screens} will be set to the
...
\libtypename{XdbeScreenVisualInfo} structures of length \argname{num\_screens}.
The {\it n}th element in the returned list corresponds to the {\it
n}th drawable in the \argname{screen\_specifiers} list, unless
\argname{num\_screens} was passed in with the value zero, in which
...
This function frees the list of \libtypename{XdbeScreenVisualInfo} returned by
the function \cfunctionname{XdbeGetVisualInfo}.
\end{keeptogether}
...
of the specified \argname{window}.  The \argname{swap\_action} is a
hint to indicate the swap action that will likely be used in
subsequent calls to \cfunctionname{XdbeSwapBuffers}.  The actual swap
action used in calls to \cfunctionname{XdbeSwapBuffers} does not have
to be the same as the \argname{swap\_action} passed to this function,
...
This function frees a drawable ID, \argname{buffer}, that was obtained
via \cfunctionname{XdbeAllocateBackBufferName}.  The \argname{buffer}
...
The argument \argname{num\_windows} specifies how many windows are to
have their buffers swapped; it is the number of elements in the
\argname{swap\_info} array.  The argument \argname{swap\_info}
...
If \argname{buffer} is not a valid \libtypename{XdbeBackBuffer},
\argname{window} is set to \literal{None}.
